...
// functions are specialized for the type of data written per vertex using
// tools from type_traits.
...
auto get_emdense_gen( tuple<bool, data>* next )
{
  return [next]( uintE ngh, bool m = false ) {
    if ( m )
      next[ngh] = make_tuple( 1, pbbs::empty() );
  };
}
...
// Version of edgeMapSparse that binary-searches and packs out blocks of
// the next frontier.
template <typename data,
          typename std::enable_if<std::is_same<data, pbbs::empty>::value,
                                  int>::type = 0>
