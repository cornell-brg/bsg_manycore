...
inline size_t countOutNgh( V* v, long vtx_id, F& f )
{
  ...
  if ( d < 2000 ) {
    ...
    appl::parallel_for_1( (size_t)0, (size_t)blocks, [&]( size_t i ) {
      size_t s  = b_size * i;
                          ...
                          uintE* tmp )
{
  ...
  if ( d < 5000 ) {
    ...
    appl::parallel_for( uintE( 0 ), d, [&]( uintE i ) {
      uintE ngh = v->getOutNeighbor( i );
      tmp[i]    = ngh;
      bits[i]   = p( vtx_id, ngh );
    } );
...
#endif
      : inNeighbors( iN ), outNeighbors( oN ), inDegree( id ),
        outDegree( od )
  {
  }
#ifndef WEIGHTED
