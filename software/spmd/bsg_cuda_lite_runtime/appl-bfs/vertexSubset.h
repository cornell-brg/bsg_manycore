...
struct vertexSubsetData {
  ...
  // Sparse
  inline uintE& vtx( const uintE& i ) const
  {
    return std::get<0>( s[i] );
  }
  inline data& vtxData( const uintE& i ) const
  {
    return std::get<1>( s[i] );
  }
  inline tuple<uintE, data> vtxAndData( const uintE& i ) const
  {
    return s[i];
  }
  ...
  inline data& ithData( const uintE& v ) const
  {
    return std::get<1>( d[v] );
  }
  ...
  auto get_fn_repr() const
  {
    ...
    if ( isDense ) {
      fn = [&]( const uintE& v ) -> Maybe<tuple<uintE, data>> {
        auto ret = Maybe<tuple<uintE, data>>(
            make_tuple( v, std::get<1>( d[v] ) ) );
...
struct vertexSubsetData<pbbs::empty> {
  ...
  {
    auto d_map =
        make_in_imap<size_t>( n, [&]( size_t i ) { return _d[i]; } );
  ...
  inline pbbs::empty vtxData( const uintE& i ) const
  {
    return pbbs::empty();
  }
  ...
  inline pbbs::empty ithData( const uintE& v ) const
  {
    return pbbs::empty();
  }
